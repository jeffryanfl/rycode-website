...
| `src/layouts/` | Site shell. Variants: `landing` (hub), `newspaper` (A.I. essay + Risk analyses), `research` (journal), `tools` (charcoal index), `risk` (charcoal Risk index), `ai` (A.I. landing, lab doors, model pages), `economics` (Economics index + essays), `journal` (default). |
...
- New Risk analysis → `src/pages/risk/<slug>.astro` on the newspaper chassis (charcoal, cyan hairlines, Newsreader H1, glossary rail). Add one row under a named section on `src/pages/risk.astro` only when it is published. Headlines live in `docs/risk/`. Do not invent copy. Do not drop a long-wall draft. Do not use a hook, bell, or carabiner for the Risk mark. Do not create `/risk/emerging/`. Live analyses: `/risk/six-percent-that-stays/`, `/risk/panic-before-the-breach/`.
...
Home is charcoal emptiness (`#080a10`): five 1px doors (Research lens SVG, Tools compass SVG, Economics chart PNG, Risk triangle PNG, Systems three-node triangle SVG) at `rgba(248, 250, 252, 0.52)`, small overhead brain, square Ry. Inter only. No dollar, shield, COSO cube, cyan, glass, or hero sentence. Systems is not a room on this site. Tools is a charcoal index, same ground as home. Research is charcoal journal. `/ai` is charcoal newspaper. `/economics` is a charcoal index. `/risk` is a charcoal index, not GRC; Risk analyses use newspaper cyan hairlines.
