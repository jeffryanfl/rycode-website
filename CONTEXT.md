...
**A.I.** Charcoal branch at `/ai`: signed brain, Inter “A.I.”, italic dek, Deep dive rows plus Models cards (Anthropic, Meta, OpenAI, TypeSafe, xAI) and a Hardware row. No cyan. Deep dive essays live on the newspaper chassis: `/ai/next-token-engine/` (RLVR) and `/ai/chat-models-write-strings-system-one-returns-decisions/` (System One / Jev). Do not put those essays on `/research`. Lab doors at `/ai/models/anthropic/`, `/ai/models/meta/` (Muse + Llama), `/ai/models/openai/`, `/ai/models/typesafe/` (Jev), and `/ai/models/xai/` (five Grok doors: Models, Grok Build, Grok Bot, Imagine, Voice). Do not invent Meta benches or Llama 3.x cards. Hardware hub at `/ai/hardware/` teaches fab ≠ campus: Terafab (chip fab) vs Memphis / Colossus (training campus). Model pages under lab paths from `docs/ai/`. Entered from the home brain. Not a Research piece and not a Tools clone. Do not invent benchmarks, average Colossus GPU counts, or call Terafab a data center.
