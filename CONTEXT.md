...
**Risk.** Charcoal `#080a10` at `/risk`, same ground as home. Warning-triangle PNG from `/landing/hub-risk.png`, Inter “Risk”. Square Ry, 1px hairline, named sections, charcoal topic chips, then rows. No cyan. No “Coming soon”. No hook, bell, or carabiner. Not a control-framework page, not L2. Analyses are charcoal digest pages at `/risk/[slug]`: Newsreader H1, then this order: decoder (term, then gloss), hero path diagram, number-card right rail, at most one emphasis, What to watch checklist, Sources. No house term before its gloss. Live: `/risk/when-the-spender-runs-the-printer/` (kicker “Debt held by the public”; closer from `/economics/post-inflation-dollars-pay-pre-inflation-debts/`), `/risk/the-ban-lands-on-open-source/` (H1: The July 2026 OpenAI eval-agent incident at Hugging Face; kicker “OpenAI / Hugging Face incident”), `/risk/six-percent-that-stays/` (kicker “Thirty-year Treasury yield”; closer “From this essay” to `/economics/ten-trillion-to-roll/`) under Emerging risks. Old `/risk/panic-before-the-breach/` 301s to the ban-lands page. Do not create `/risk/emerging/`. Headlines live in `docs/risk/`.
